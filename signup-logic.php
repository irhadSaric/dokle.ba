...
if(isset($_POST['reg']))
{
	if(!empty($_POST['Ime']) && !empty($_POST['Prezime']) && !empty($_POST['email']) && !empty($_POST['pass']) && !empty($_POST['pass2']))
	{
		...
		if($imaLiGa)
		{
			header("location:signup.php?poruka=Mejl je vec u upotrebi");
		}
		else{
	...
	{
		header("location:signup.php?poruka=Unesite sva polja");
	}
}
else 
{
	header("location:signup.php?poruka=Kardinalna greska!");
}
